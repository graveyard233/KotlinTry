...
suspend fun <T> BaseViewModel.handleRequest(
    response: BilibiliMsg<T>,
    successBlock: suspend CoroutineScope.(response: BilibiliMsg<T>) -> Unit = {},
    errorBlock: suspend CoroutineScope.(response: BilibiliMsg<T>) -> Boolean = { false }
) {
    coroutineScope {
        when (response.code) {
...
//                    errorResponse.value = response
                    Log.e("handleRequest", "handleRequest: error block")
                }
            }
        }
    }
